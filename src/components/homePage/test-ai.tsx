...
export default function AIChat() {
  ...
  return (
    <div className=" mx-auto">
      <Card className="flex flex-col shadow-2xl border-t-4 border-t-emerald-500 lg:w-96 lg:h-96 max-h-[500px] ">
        <CardHeader className="bg-gradient-to-r from-emerald-500 to-teal-600 text-white py-2 lg:py-4">
        ...
        <ScrollArea ref={scrollAreaRef}  className="flex-1 p-2 bg-slate-50">
            ...
            {chat.length === 0 ? (
              ...
              chat.map((message, index) => (
                <div key={index} className={cn("flex", message.from === "you" ? "justify-end" : "justify-start")}>
                  <div
                    className={cn(
                      "flex items-start gap-0.5 max-w-[90%] ",
                  ...
                  >
                    ...
                    >
                      <div className="whitespace-pre-wrap text-sm lg:text-base">{message.text}</div>
                    </div>
                  </div>
                </div>
              ))
