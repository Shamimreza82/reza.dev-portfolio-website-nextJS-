...
type Props = {
  phone: string;              // "8801531297879"
  message?: string;           // optional preset
  position?: "fixed" | "absolute";
  className?: string;
};

export default function CombinedFloat({
  phone,
  ...
  return (
    <>
      {/* The Floating Action Button with a 2-option menu */}
      <TooltipProvider>
        <DropdownMenu>
          <Tooltip>
            <DropdownMenuTrigger asChild>
              <TooltipTrigger asChild>
                <Button
                  ...
                  size="icon"
                  className={cn(
                    position === "fixed" ? "fixed bottom-5 right-5" : "absolute bottom-5 right-5",
                    "h-14 w-14 rounded-full z-[60] shadow-lg",
          ...
          <DropdownMenuContent align="end" sideOffset={10} className="w-56">
            ...
            <DropdownMenuItem asChild>
              ...
              >
                {/* WhatsApp SVG so you don’t need extra icon libs */}
      ...
      {/* Dialog to show your RightSocialRail when chosen */}
      <Dialog open={socialOpen} onOpenChange={setSocialOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="py-2">
            {/* You can keep your RightSocialRail as-is */}
            <RightSocialRail />
          </div>
        </DialogContent>
      </Dialog>
